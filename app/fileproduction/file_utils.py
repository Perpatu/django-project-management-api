...
def task(file_id, users, destiny, where):
    """Prepare data for task in project websocket"""
...
def task_department(current_task, users):
    """Prepare data for task in department websocket"""
    current_task_query = Task.objects.get(id=current_task.id)
    current_task_serializer = serializers.TaskDepartmentSerializer(current_task_query, many=False)
    current_task_data = current_task_serializer.data

    if current_task_data['previous_task'] is not None:
        previous_task_query = Task.objects.get(id=current_task_data['previous_task']['id'])
        previous_task_serializer = serializers.TaskDepartmentSerializer(previous_task_query, many=False)
        previous_task_data = previous_task_serializer.data
    else:
        previous_task_data = None

    if current_task_data['next_task'] is not None:
        next_task_query = Task.objects.get(id=current_task_data['next_task']['id'])
        next_task_serializer = serializers.TaskDepartmentSerializer(next_task_query, many=False)
        next_task_data = next_task_serializer.data
    else:
        next_task_data = None

    project_progress(current_task.file.project.id)

    message = {
        'previous_task': previous_task_data,
